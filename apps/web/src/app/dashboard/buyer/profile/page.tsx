    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-1">Manage your corporate identity and billing details.</p>
        </div>
      ...
      <div className="bg-surface border border-border-subtle rounded-xl p-6">
        <div className="flex items-center space-x-6 mb-6 pb-6 border-b border-border-subtle">
          <div className="h-20 w-20 rounded-full bg-primary-50 border border-primary-100 flex items-center justify-center">
            <span className="text-2xl font-bold text-primary-600">
              {profile?.company?.name?.substring(0, 2).toUpperCase() || profile?.name?.substring(0, 2).toUpperCase() || 'CB'}
            </span>
          </div>
          <div>
            <h3 className="text-lg font-bold text-text-primary">{profile?.company?.name || profile?.name || '-'}</h3>
            {profile?.company && (
              <p className="text-sm text-text-secondary mt-1">{profile.company.type} • {profile.company.employeeCount} Employees</p>
            )}
            <div className="flex items-center mt-3 space-x-3">
              <span className="inline-flex items-center px-2.5 py-1 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-600 border border-emerald-100">Verified Entity</span>
              <span className="text-xs font-medium text-text-tertiary">Joined {new Date(profile?.createdAt).toLocaleDateString()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <h4 className="text-xs uppercase tracking-wide font-semibold text-text-tertiary mb-3">Contact Information (Personal)</h4>
            <div className="space-y-4">
              <div>
                <p className="text-xs font-medium text-text-secondary">Full Name</p>
                <p className="text-sm font-medium text-text-primary mt-1">{profile?.name || '-'}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-text-secondary">Primary Email</p>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-text-primary mt-1">{profile?.mobile || '-'}</p>
              </div>
              <div>
                <p className="text-xs font-medium text-text-secondary">Personal Address</p>
                <p className="text-sm font-medium text-text-primary mt-1">{profile?.address}, {profile?.city} - {profile?.pincode}</p>
              </div>
            </div>
          </div>
          <div>
            <h4 className="text-xs uppercase tracking-wide font-semibold text-text-tertiary mb-3">Company Details</h4>
            <div className="space-y-4">
              {profile?.company ? (
                <>
                  <div>
                    <p className="text-sm font-medium text-text-secondary">Headquarters / Billing Address</p>
                ...
                <p className="text-sm text-text-secondary">No company associated.</p>
              )}
            </div>
          </div>
        </div>
      ...
      {/* Reviews Section */}
      <div className="bg-surface border border-border-subtle rounded-xl p-6">
        <h3 className="text-lg font-bold text-text-primary mb-4">Reviews from Suppliers</h3>
        {stats && stats.totalReviews > 0 ? (
          <div className="mb-6 flex items-center gap-4 p-4 bg-surface-raised rounded-xl">
            <div className="text-center">
              <p className="text-3xl font-black text-text-primary">{stats.averageRating}</p>
              ...
              <p className="text-xs text-text-secondary mt-1">{stats.totalReviews} Ratings</p>
            </div>
              ...
              {[5, 4, 3, 2, 1].map(star => {
                ...
                return (
                  <div key={star} className="flex items-center gap-2 text-xs">
                    <span className="w-2 font-medium text-text-secondary">{star}</span>
          ...
          <p className="text-sm text-text-secondary mb-4">No ratings received yet.</p>
        )}
          ...
          <div className="relative w-full max-w-3xl bg-surface max-h-[90vh] rounded-2xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-border-subtle flex justify-between items-center bg-surface z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-text-primary tracking-tight">Edit Profile</h2>
                  <p className="text-xs text-text-secondary mt-0.5">Update your corporate details</p>
                </div>
              </div>
              ...
              <form id="edit-profile-form" onSubmit={handleSave} className="grid md:grid-cols-2 gap-6">
                ...
                <div className="bg-surface border border-border-subtle rounded-2xl p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-bold text-text-primary uppercase tracking-wide">Personal Details</h3>
                  </div>
                  <div className="space-y-4">
                    <div>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-text-secondary mb-1.5">Personal Address</label>
                ...
                <div className="bg-surface border border-border-subtle rounded-2xl p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-bold text-text-primary uppercase tracking-wide">Company Details</h3>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-text-secondary mb-1.5">Company Name</label>
                      <input value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} type="text" className="w-full px-4 py-2.5 bg-surface-raised border border-border-subtle rounded-xl text-sm text-text-primary focus:border-primary-500 focus:bg-surface focus:ring-4 focus:ring-primary-500/10 outline-none transition-all" placeholder="Registered company name" />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-text-secondary mb-1.5">Headquarters Address</label>
                      <textarea value={formData.companyAddress} onChange={e => setFormData({...formData, companyAddress: e.target.value})} rows={2} className="w-full px-4 py-2.5 bg-surface-raised border border-border-subtle rounded-xl text-sm text-text-primary focus:border-primary-500 focus:bg-surface focus:ring-4 focus:ring-primary-500/10 outline-none transition-all resize-none" placeholder="Full corporate address"></textarea>
                    </div>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-semibold text-text-secondary mb-1.5">Company Type</label>
                        <div className="relative">
                          <select value={formData.companyType} onChange={e => setFormData({...formData, companyType: e.target.value})} className="w-full px-4 py-2.5 bg-surface-raised border border-border-subtle rounded-xl text-sm text-text-primary focus:border-primary-500 focus:bg-surface focus:ring-4 focus:ring-primary-500/10 outline-none transition-all appearance-none">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-text-secondary mb-1.5">Employee Count</label>
                        <div className="relative">
                          <select value={formData.employeeCount} onChange={e => setFormData({...formData, employeeCount: e.target.value})} className="w-full px-4 py-2.5 bg-surface-raised border border-border-subtle rounded-xl text-sm text-text-primary focus:border-primary-500 focus:bg-surface focus:ring-4 focus:ring-primary-500/10 outline-none transition-all appearance-none">
            ...
            <div className="p-5 border-t border-border-subtle bg-surface flex justify-end gap-3 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
              ...
              <button type="submit" form="edit-profile-form" disabled={saving} className="px-5 py-2.5 text-sm font-semibold text-white bg-primary-500 rounded-xl hover:bg-primary-600 transition-all shadow-md shadow-primary-500/20 disabled:opacity-50 disabled:shadow-none flex items-center justify-center min-w-[120px]">
                {saving ? (
